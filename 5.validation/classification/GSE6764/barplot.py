...
file_names = ['class_count1.csv', 'class_count2.csv', 'class_count3.csv', 'class_count4.csv', 'class_count5.csv', 'class_count6.csv', 'class_count7.csv']
...
colors = ['#DEECF6', '#AFC8E2', '#E2F2CD', '#B6DAA7', '#E8E0EF',  '#C2B1D7', '#F9D5D5', '#E89DA0']
...
def plot_metric(metric_name, values, stds):
    ...
    plt.title(f'{metric_name} of Time Series', fontsize=18, pad=60)  # Adjust the pad parameter to move the title higher
    ...
    for bar, val, std in zip(bars, values, stds):
        yval = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2, yval + 0.04, f'{round(yval, 4)}', ha='center', fontsize=13)
    plt.tight_layout()
    plt.savefig(f'{metric_name}_scores.png')
    plt.savefig(f'{metric_name}_scores_2.pdf', format="pdf")
    plt.show()
