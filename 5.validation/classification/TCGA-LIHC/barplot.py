...
def plot_metric(metric_name, values, stds):
    ...
    bars = plt.bar(range(len(x_labels)), values, yerr=stds, capsize=5, color=colors, width=0.5)
    ...
    for bar, val, std in zip(bars, values, stds):
        yval = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2, yval + 0.13, f'{round(yval, 4)}', ha='center', fontsize=13)
    plt.tight_layout()
    plt.savefig(f'{metric_name}_scores.png')
    plt.savefig(f'{metric_name}_scores.pdf', format="pdf")
    plt.show()
